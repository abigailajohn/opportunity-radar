...
class DeterministicOpportunityExtractor:
    ...
    def extract(self, page: FetchedPage) -> Opportunity:
        ...
        cycle_year_match = re.search(r"\bCycle year\s*:\s*(20\d{2})\b", text, re.I)
        program_family = family_match.group(1).strip() if family_match else None
        cycle_label = cycle_match.group(1).strip() if cycle_match else None
        cycle_year = int(cycle_year_match.group(1)) if cycle_year_match else None
